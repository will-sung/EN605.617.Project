...
struct DeviceGrayImage {
    int      width  = 0;
    int      height = 0;
    uint8_t* d_data = nullptr;  // device memory
};
...
DeviceGrayImage npp_rgba_to_gray(const uint8_t* h_rgba, int width, int height);
...
// stage 5 (CUDA): label connected components in a binary edge map
// components smaller than min_area pixels are treated as background
CCLResult ccl_label(const DeviceGrayImage& binary, int min_area = CCL_MIN_AREA);
